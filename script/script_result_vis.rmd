...
Figure_1A = ((Initial_dateJ + labs(title = "A")))+ mid_dateJ+later_dateJ+ 
   Initial_date +mid_date +later_date 
...
ggsave("Figure1.png", plot = Figure_1, width = 25, height = 30, units = "cm", bg = 'transparent',  dpi = 300,  scale = 0.85)
...
error_quant_30 = error_dataset %>%
  ...
  facet_wrap(~location) +
  bbc_style()+
  labs(linetype="Median Quantile", color = "Model", shape = "")+
  scale_linetype_manual(values = c("dotted","solid","dashed"),
                        labels = c("25th Quantile","Median", "75th Quantile" ))+
  scale_color_manual(values=model_colors,
                      breaks=c("GARW", "FGA","MLR" ,"Piantham","Naïve"))+
  labs( x = "Days from date of estimation", y= "log(MAE)")+
  theme(axis.title.x = element_text(color="Black", size=14),
        axis.title.y = element_text(color="Black", size=14),
        panel.grid.major = element_line(),
        axis.line = element_line(colour = "black"),
  ...
  scale_y_continuous(trans = log_trans(),
                     ...
                     labels = label_percent(accuracy = 0.1L))+
...
mae_dist = error_dataset %>%
  ...
  group_by(lead, location, model) %>%
  ggplot(aes(y = MAE, x = factor(model, levels = c("Naïve","Piantham","MLR","FGA","GARW")), fill = model)) +
  geom_violin(lwd = 0.4,
              trim=FALSE, 
             position = "dodge", 
              alpha = 0.825,
            ...
            show.legend = TRUE,
            inherit.aes = TRUE)+
  stat_summary(aes(x = factor(model), y = MAE,col = "Mean"), fun= mean,
               geom = "crossbar", size = 0.3, alpha = 1, na.rm = TRUE, inherit.aes = FALSE)+
  scale_colour_manual(name = "Mean Quantile",
    values = c("Mean" = "black",
                                 "Q3" = "black",
                                 "Q1" = "black"),
                       breaks = c('Mean','Q3','Q1'))+
 ...
 scale_alpha(guide = 'none')  +
  scale_y_continuous(limit = c(-0.2,0.45),
                     breaks = c(0,0.2,0.4))
...
ggsave("Figure2tt.png", plot = Figure_2, width = 25, height = 30, units = "cm", bg = 'white',  dpi = 300,  scale = 0.85)
...
#Figure 4: Errors with volume of sequences: Average MAE plot with sequences 
...
get_recent_sequences_frac(seq_date, "2022-04-01", 14)
seq
...
get_recent_sequences_frac_pivot <- function (df, end_date, k) {
  filtered <- df %>%
    filter((as.Date(end_date) - k) < as.Date(date) & (as.Date(end_date) > as.Date(date))) %>%
    group_by(location) %>%
    summarise(
...
get_recent_sequences_frac_pivot(seq_date, "2022-04-15", 14)
...
for (pivot_date in pivot_dates) {
  ...
  seq_d = get_recent_sequences_frac_pivot(seq_date, last_date , 14) %>%
...
mae_seq_frac_data_piv = average_mae %>%
  left_join(frac_seq_pivot)



#Mean delays 

get_mean_delays_piv <- function (df, end_date, k) {
  filtered <- df %>%
    filter((as.Date(end_date) - k) < as.Date(date) & (as.Date(end_date) > as.Date(date))) %>%
    mutate(delay = as.Date(date_submitted) - as.Date(date)) %>%
    group_by(location) %>%
...
for (pivot_date in pivot_dates) {
  last_date = as.Date(pivot_date) - 14
  seq_d = get_mean_delays_piv(seq_date, last_date , 14) %>%
    mutate(pivot_date = pivot_date)
...
#Figure 5: Variant Selection Pressure with MAE
...
average_mae =error_dataset %>%
  group_by(location) %>%
  #filter(location =="Japan")%>%
  select(location, variant, MAE, date, raw_freq) %>%
  aggregate(MAE~date+location, FUN=mean) %>%
  mutate(date = as.Date(date))
...
class(select_pres_data2$selective_pres)


cor.test(select_pres_data2$selective_pres, select_pres_data2$MAE_avg, method = "spearman")

plot(lm(select_pres_data2$MAE_avg ~ select_pres_data2$selective_pres))



m0 = lm ( MAE ~ 1 ,full_test )
m1 = lm (MAE_avg ~ selective_pres ,select_pres_data2)
m2 = lm ( MAE_avg ~ selective_pres + I(selective_pres^2), select_pres_data2 )
m3 = lm ( MAE_avg ~ selective_pres + I(selective_pres^2) + I(selective_pres^3) , select_pres_data2)
m4 = lm ( MAE_avg ~ selective_pres + I(selective_pres^2) + I(selective_pres^3) + I(selective_pres^4) , select_pres_data2)
AIC(m0,m1,m2,m3,m4)
...
full_test = mae_seq_full %>%
  right_join(select_pres_data2)
...
reg = lm(meanMAE ~ frac_seq_avail + seq_available + lead +mean_delay + median_delay + selective_pres + frac_seq_avail *  mean_delay, test3)
