...
seq_qc <- read.csv("../notebooks/seq_counts_qc.tsv")
...
med_del_corr = full_model_data %>%
 ...
 summarise(median_delay = median(median_delay),
  ...
  ggplot(aes(x = median_delay, y = mean_nowcast_MAE, group = location, color = location))+
  geom_point(size = 2)+
   scale_color_manual(values = loc_color_map)+
  stat_cor(method = "pearson", aes(x = median_delay, y = mean_nowcast_MAE), label.x = 65, inherit.aes = F)+
  theme_minimal()+
...
frac_seq_corr = full_model_data %>%
  ...
  summarise(frac_seq_avail = median(frac_seq_avail),
            mean_nowcast_mae = median(meanMAE)) %>%
  ggplot(aes(x = frac_seq_avail, y = mean_nowcast_mae, group = location, color = location))+
  geom_point(size = 2)+
  stat_cor(method = "pearson", aes(x = frac_seq_avail, y = mean_nowcast_mae), label.x = 0.2, inherit.aes = F)+
  theme_minimal()+
   scale_color_manual(values = loc_color_map)+
...
pivot_mae = full_model_data %>%
  ...
  summarise(#seq_collected = median(seq_collected),
            mean_nowcast_MAE = median(meanMAE)) %>%
  ggplot(aes(x = pivot_date, y = mean_nowcast_MAE, group = location, color = location))+
  geom_line()+
...
country_mae = full_model_data %>%
  ...
  summarise(#seq_collected = median(seq_collected),
            mean_nowcast_MAE = median(meanMAE)) %>%
  ggplot(aes(x = location, y = mean_nowcast_MAE, group = location, color = location))+
  #geom_point(size = 2)+
