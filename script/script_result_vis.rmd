...
full_test %>%
  ...
  geom_point(aes(y = meanMAE, x= sequence_recent, color =location), size = 0.5)+
  #geom_smooth(aes(y = MAE, x= sequence_recent), method = "lm")+
  theme_classic()+
  stat_poly_eq(aes( x = sequence_recent, y =meanMAE ), formula = y ~ x, size = 5) +
  stat_poly_line(aes( x = sequence_recent, y =meanMAE ), formula = y ~ x) +
