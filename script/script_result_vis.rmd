...
error_dataset = read.csv('../errors/model_scores.tsv', sep="\t") %>%
  mutate(model = recode(model, naive = "Naïve")) 



table(error_dataset$model)

# Read full rank CI error dataset
error_dataset_fullrank = read.csv('../errors_fullrank/model_scores_output.csv') %>%
  mutate(model = recode(model, naive = "Naïve")) 

table(error_dataset_fullrank$location)
...
loc_color_map = c("USA" = "#a12525", "United Kingdom" = "#00299b", "Japan" = "#3d405b",
                  ...
                  "Vietnam" = "#ffbb00", "Trinidad and Tobago" = "#1e90ff")

model_colors = c("Naïve"= "#5E0035", "Piantham" ="#17BEBB", "MLR" = "#003049", "FGA" = "#FFCF00","GARW" = "#D62828" , "Hierarchical-MLR" = "#2ECC71")
...
error_dataset %>%
  group_by(lead, location, model) %>%
  filter(model != "Hierarchical-MLR") %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","Trinidad and Tobago", "United Kingdom","USA", "Vietnam" )) %>%
  filter(lead > -30 & lead < 30) %>%
  ...
  labs(x = "Days from date of estimation", y = "Median Absolute Error") +
...
mae_nat_error_30 =# Mean MAE with natural scale
error_dataset %>%
  group_by(lead, location, model) %>%
  filter(model != "Hierarchical-MLR") %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","Trinidad and Tobago", "United Kingdom","USA", "Vietnam" )) %>%
  ...
  annotate(geom = "rect", xmin = -30, xmax = 0, ymin = 0, ymax = 0.2,
           color = NA, fill = "#bdc3c7", alpha = 0.15) +
  annotate(geom = "text", x = -18, y = 0.15, label = "Nowcast", size = 3, fontface = 4) +
  annotate(geom = "rect", xmin = 0, xmax = 30, ymin = 0, ymax = 0.2,
           color = NA, fill = "#2c3e50", alpha = 0.15) +
  annotate(geom = "text", x = 18, y = 0.15, label = "Forecast", size = 3, fontface = 4)  + geom_hline(yintercept = 0.05, linetype = "dashed")
...
error_dataset %>%
  group_by(lead, location, model) %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","United Kingdom", "Vietnam","USA",  "Trinidad and Tobago")) %>%
  filter(lead > -30 & lead < 30) %>%
  summarise(quant75 = quantile(MAE, probs = c(0.75), na.rm = TRUE),
            quant50 = quantile(MAE, probs = c(0.50), na.rm = TRUE),
            quant25 = quantile(MAE, probs = c(0.25), na.rm = TRUE),
...
error_dataset %>%
  ...
  scale_x_continuous(breaks = c(-30, -14, 0, 14, 30)) +
    scale_y_continuous(
    limits = c(0, 0.3), # This sets the limits from 0% to 5%
    breaks = seq(0, 0.3, by = 0.05), # This sets the breaks at every 1% increment
    labels = scales::percent(seq(0, 0.3, by = 0.05)) # This sets the labels to show the percentages
  ) +  # Set y-axis breaks and labels
  theme(axis.title.x = element_text(color = "Black", size = 14),
        axis.title.y = element_text(color = "Black", size = 14),
        panel.grid.major = element_line(),
        ...
        legend.position = "right",
        legend.title = element_text(size = 12)) +
  annotate(geom = "rect", xmin = -30, xmax = 0, ymin = 0, ymax = 0.3,
           color = NA, fill = "#bdc3c7", alpha = 0.15) +
  annotate(geom = "text", x = -18, y = 0.25, label = "Nowcast", size = 3, fontface = 4) +
  annotate(geom = "rect", xmin = 0, xmax = 30, ymin = 0, ymax = 0.3,
           color = NA, fill = "#2c3e50", alpha = 0.15) +
  annotate(geom = "text", x = 18, y = 0.25, label = "Forecast", size = 3, fontface = 4)  + geom_hline(yintercept = 0.05, linetype = "dashed")
...
Figure_2A=  (med_log_error_30 + labs(title = element_text( "A",face = "bold"))) / (mae_nat_error_30 +labs(title = element_text( "B",face = "bold"))) 
...
ggsave("model_comp_A.png", plot = Figure_2A, width = 20, height = 30, units = "cm", bg = 'white',  dpi = 300)
ggsave("model_comp_B.png", plot = Figure_2B, width = 20, height = 15, units = "cm", bg = 'white')
...
table1 = error_dataset %>%
  ...
  setDT() %>%
  pivot_wider(names_from = model, values_from = c(Median, Mean)) %>% mutate(across(starts_with("Median_") | starts_with("Mean_"), 
                ~ sprintf("%.1f%%", .*100)))


model_comp_table = table1 %>%
      mutate(lead = paste(lead, "Lead from date of estimation")) %>%
    group_by(lead)  %>%
  gt() %>%
    tab_spanner(
    label = "Median Absolute Error",
    columns = c(Median_Naïve,Median_Piantham, Median_MLR, Median_FGA, Median_GARW)
  ) %>%
  tab_spanner(
    label = "Mean Absolute Error",
    columns = c(Mean_Naïve, Mean_Piantham, Mean_MLR, Mean_FGA, Mean_GARW)
  ) %>%
  tab_spanner(
    label = "Models",
    columns = c(Median_Naïve,Median_Piantham, Median_MLR, Median_FGA, Median_GARW,
                Mean_Naïve, Mean_Piantham, Mean_MLR, Mean_FGA, Mean_GARW )
  ) %>%
  cols_label(
    location = "Location",
    Mean_FGA = "FGA",
    ...
    Mean_MLR = "MLR",
    Mean_Naïve = "Naïve",
    Mean_Piantham = "Piantham",
    Median_FGA = "FGA",
    Median_GARW = "GARW",
    Median_MLR = "MLR",
    Median_Naïve = "Naïve",
    Median_Piantham = "Piantham"
  ) %>%

#-30 LEAD
    tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c( Median_GARW , Mean_FGA, Mean_GARW, Mean_MLR, Mean_FGA), 
                                   rows = 1)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_Piantham, Mean_FGA), 
                                   rows = 2)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_Piantham,  Mean_FGA, Mean_GARW, Mean_Piantham, Mean_MLR), 
                                   rows = 3)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_FGA, Median_GARW, Mean_GARW), 
                                   rows = 4)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_Piantham, Mean_MLR), 
                                   rows = 5)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_Piantham,Median_FGA, Median_GARW,Median_MLR, Mean_MLR), 
                                   rows = 6)) %>%
  tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_Piantham,Median_FGA, Median_GARW,Median_MLR, Mean_GARW), 
                                   rows = 7)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_GARW, Mean_GARW), 
                                   rows = 8)) %>%
...
#0 LEAD
      ...
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c( Median_GARW,Mean_MLR, Mean_FGA  ), 
                                   rows = 10)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_MLR,Median_GARW, Mean_MLR ), 
                                   rows = 11)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_GARW, Mean_FGA), 
                                   rows = 12)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_GARW, Mean_MLR), 
                                   rows = 13)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_GARW,Median_FGA, Mean_MLR), 
                                   rows = 14)) %>%
    tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_GARW, Mean_GARW), 
      ...
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_FGA, Mean_FGA), 
                                   rows = 16)) %>%
#30 LEAD
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
      ...
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_FGA, Median_MLR, Mean_GARW), 
                                   rows = 19)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_GARW, Median_MLR, Mean_FGA), 
                                   rows = 20)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_FGA, Median_MLR, Mean_MLR), 
                                   rows = 21)) %>%
      tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c(Median_MLR, Mean_MLR), 
                                   rows = 22)) %>%tab_style(style = list(
                         cell_text(weight = 'bold')), 
            locations = cells_body(columns= c( Median_GARW, Mean_MLR), 
                                   rows = 23)) %>%
      tab_style(style = list(
...
model_comp_table %>%
  gtsave(filename = "model_comp_table.png", expand = 10, vwidth = 1500, vheight = 1200)
...
med_del_corr = full_model_data %>%
  ...
  ggplot(aes(x = median_delay, y = AE, group = location, color = location))+
  geom_point(size = 2)+
   scale_color_manual(values = loc_color_map)+
  stat_cor(method = "pearson", aes(x = median_delay, y = AE), label.x = 65, inherit.aes = F,
           cor.coef.name ="r")+
  theme_minimal()+
        labs( x = "Median Submission Delay", y= "AE")+
  theme(axis.title.x = element_text(color="Black", size=14),
        axis.title.y = element_text(color="Black", size=14),
        panel.grid.major = element_line(),
        axis.line = element_line(colour = "black"),
...
frac_seq_corr = full_model_data %>%
  ...
  #summarise(frac_seq_avail = median(frac_seq_avail),
   #         mean_nowcast_mae = median(meanMAE)) %>%
  ggplot(aes(x = frac_seq_avail, y = AE, group = location, color = location))+
  geom_point(size = 2)+
  stat_cor(method = "pearson", aes(x = frac_seq_avail, y = AE), label.x = 0.2, inherit.aes = F,
           cor.coef.name = "r")+
  theme_minimal()+
   scale_color_manual(values = loc_color_map)+
        labs( x = "Fraction of Sequences at Observation", y= "AE")+
  theme(axis.title.x = element_text(color="Black", size=14),
        axis.title.y = element_text(color="Black", size=14),
        panel.grid.major = element_line(),
        axis.line = element_line(colour = "black"),
...
seq_avail_corr = full_model_data %>%
  ...
  ggplot(aes(x = seq_available+1, y = AE, group = location, color = location))+
  geom_point(size = 2)+
  stat_cor(method = "pearson", aes(x = seq_available+1, y = AE), label.x = 2.5, inherit.aes = F,
           cor.coef.name = "r")+
  theme_minimal()+
        labs( x = "Sequences Available at Observation ", y= "AE")+
   scale_color_manual(values = loc_color_map)+
  theme(axis.title.x = element_text(color="Black", size=14),
        axis.title.y = element_text(color="Black", size=14),
        panel.grid.major = element_line(),
...
country_mae = full_model_data %>%
  ...
  #summarise(#seq_collected = median(seq_collected),
  #          mean_nowcast_MAE = median(meanMAE)) %>%
  ggplot(aes(x = location, y = AE, group = location, color = location))+
  #geom_point(size = 2)+
...
qc_mae = full_model_data %>%
  ...
  #summarise(#seq_collected = median(seq_collected),
  #          mean_nowcast_MAE = median(meanMAE)) %>%
  ggplot(aes(x = prop_qc_bad, y = AE, group = location, color = location))+
  geom_point(size = 2)+
    #geom_beeswarm(aes(color= location), cex = 2, size = 1.5, alpha = 0.9,
     #           priority = "density",
    #            show.legend = T, groupOnX = T)+
  stat_cor(method = "pearson", aes(x = prop_qc_bad, y = AE), label.x = 0.2, inherit.aes = F,
           cor.coef.name = "r")+
  theme_minimal()+
  scale_color_manual(values = loc_color_map)+
        labs( x = "Proportion of Bad QC sequences", y= "AE")+
  theme(axis.title.x = element_text(color="Black", size=14),
        axis.title.y = element_text(color="Black", size=14),
        panel.grid.major = element_line(),
        axis.line = element_line(colour = "black"),
...
Figure_4_full = grid.arrange(patchworkGrob(Figure_4A / Figure_4b), left = "Absolute Error")




ggsave("Var_of_interest.png", plot = Figure_4_full, width = 38, height = 30, units = "cm", bg = 'white',  dpi = 300,  scale = 0.85)
...
#Figure 6: Pooled MlR and MLR
```{r}

# make y percent with hline 5% showing the error
med_log_error_30_pooled6 = 
# Median MAE with log scale
error_dataset %>%
  filter(model %in% c("Naïve", "MLR", "Hierarchical-MLR")) %>%
  group_by(lead, location, model) %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","Trinidad and Tobago", "United Kingdom","USA", "Vietnam" )) %>%
  filter(lead > -30 & lead < 30) %>%
  summarise(quant75 = quantile(MAE, probs = c(0.75), na.rm = TRUE),
            ...
            quant25 = quantile(MAE, probs = c(0.25), na.rm = TRUE),
            meanMAE = mean(MAE)) %>%
  ggplot(aes(x = lead, color = factor(model, levels = c("Naïve",  "MLR", "Hierarchical-MLR")))) +
  geom_line(aes(y = quant50), size = 0.5) +
  #geom_line(aes(y =meanMAE ), alpha = 0.75)+
  #geom_point(aes(y = quant50), size = 0.9, alpha = 0.75) +
  facet_wrap(~location) +
  ...
  labs(linetype = " ", color = "Model", shape = 1) +
  scale_color_manual(values = model_colors,
                     breaks = c("Naïve",  "MLR", "Hierarchical-MLR")) +
  labs(x = "Days from date of estimation", y = "Median Absolute Error") +
  scale_x_continuous(breaks = c(-30, -14, 0, 14, 30)) +
  theme(axis.title.x = element_text(color = "Black", size = 14),
        axis.title.y = element_text(color = "Black", size = 14),
...
mae_nat_error_30_pooled6 =# Mean MAE with natural scale
error_dataset %>%
  filter(model %in% c("Naïve", "MLR", "Hierarchical-MLR")) %>%
  group_by(lead, location, model) %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","Trinidad and Tobago", "United Kingdom","USA", "Vietnam" )) %>%
...
Figure_6=  (med_log_error_30_pooled6 + labs(title = element_text( "A",face = "bold"))) / (mae_nat_error_30_pooled6 +labs(title = element_text( "B",face = "bold"))) 


ggsave("model_comp_PooledMLR.png", plot = Figure_6, width = 20, height = 30, units = "cm", bg = 'white',  dpi = 300)
```
...
## Figure 1A Supplementary Function


```{r}

# supplemntary figure 1A Function


plot_country_sequences <- function(country, seqs_first, seq_mid, seq_mid2, seq_last, ylim) {
  initial_date <- seqs_first %>%
  ...
  later_date <- seq_last %>%
   ...
   plot_grid(
    initial_date + theme(legend.position = "none"),
    mid_date + theme(legend.position = "none"),
    mid_date2 + theme(legend.position = "none"),
    later_date + theme(legend.position = "none"),
    nrow = 1
  )
}
...
## Figure 1B Sup Function


```{r}

# supplemntary figure 1B Function

plot_error_dataset <- function(error_dataset, model_input, country_input) {
  
  error_dataset %>%
    filter(lead > -30 & lead < 30) %>%
    filter(location == country_input & model == model_input, na.rm = TRUE) %>%
    filter(variant != "other" & variant != "Delta" & variant != "Omicron 23A") %>%
    ggplot() +
    geom_line(aes(x = as.Date(date), y = smoothed_freq), color = "black", size = 1.5, alpha = 0.9) +
    geom_line(aes(x = as.Date(date), y = pred_freq, group = factor(pivot_date), color = factor(variant), fill = factor(pivot_date)), size = 0.8, alpha = 0.7) +
    facet_wrap(~variant, nrow = 1,
               labeller = labeller(variant = c(
                 "Omicron 21K" = "21K (BA.1)",
                 "Omicron 21L" = "21L (BA.2)",
                 "Omicron 22A" = "22A (BA.4)",
                 "Omicron 22B" = "22B (BA.5)",
                 "Omicron 22C" = "22C (BA.2.12.1)",
    ...
    scale_color_manual(
      values = variant_colors,
      limits = c(
        "Omicron 21K", "Omicron 21L", "Omicron 22A", "Omicron 22B", "Omicron 22C",
        "Omicron 22D", "Omicron 22E", "Omicron 23A"
      ),
      labels = c("Clade 21K", "Clade 21L", "Clade 22A", "Clade 22B", "Clade 22C", "Clade 22D",
                 "Clade 22E", "Clade 23A")
    ...
    labs(
      x = "Date", y = "Frequency",
      caption = "MA smoothed frequency with predicted frequency",
      title = paste(country_input, "-", model_input)
    ) +
    theme(
    ...
    ylim(0, 1) +
    scale_x_date(date_breaks = "4 month", labels = label_date_short()) +
    scale_y_continuous(labels = label_percent()) +
...
models <- c("Piantham","MLR", "FGA", "GARW")
countries = "USA"
for (country in countries) {
  plot_list <- list()  # Initialize a list to store plots for this country
  
  for (model in models) {
    plot <- plot_error_dataset(error_dataset, model, country)
    plot_list[[model]] <- plot
  }
  
  # Stack the plots vertically for this country
  combined_plot <- grid.arrange(grobs = plot_list, ncol = 1)
  
  # Save the plot with appropriate name
  file_name <- paste0("supplementary_fig_", country, ".png")
  ggsave(file_name, combined_plot, width = 20, height = 15)
}
...
```{R}
countries = c("Australia", "Brazil","South Africa", "Trinidad and Tobago", "United Kingdom", "Vietnam")
ylim_values <- c(750, 750, 250, 50, 15000, 100)
# Call the function for each country
plot_all_countries_sequences <- function(countries, seqs_first, seq_mid, seq_mid2, seq_last, ylim_values) {
  country_plots <- list()
  
  for (i in seq_along(countries)) {
    country <- countries[i]
    ylim <- ylim_values[i]
    country_plot <- plot_country_sequences(country, seqs_first, seq_mid, seq_mid2, seq_last, ylim)
    
    # Add country name as subtitle to the plot
    country_plot <- country_plot + theme(plot.subtitle = element_text(size = 12,
                                                                      hjust = 0, vjust = 1, margin = margin(5, 5, 5, 5, "pt"), face = "bold"))
    country_plot <- country_plot + labs(subtitle = country)
    
    country_plots[[country]] <- country_plot
  }
  
  arranged_plots <- wrap_plots(country_plots, ncol = 1)
...
supp1_fig = plot_all_countries_sequences(countries, seqs_first, seq_mid, seq_mid2, seq_last, ylim_values)


ggsave("Supplementary_Fig_1A.png", plot = supp1_fig, width = 40, height = 30, units = "cm" )
...
coverage_supp_2_A = error_dataset %>%
  group_by(lead, location, model) %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","Trinidad and Tobago", "United Kingdom","USA", "Vietnam" )) %>%
  filter(model %in% c("Naïve", "Piantham", "MLR","Hierarchical-MLR", "FGA", "GARW")) %>%
  filter(lead > -30 & lead < 30) %>%
  summarise(quant75 = quantile(MAE, probs = c(0.75), na.rm = TRUE),
            quant50 = quantile(MAE, probs = c(0.50), na.rm = TRUE),
            quant25 = quantile(MAE, probs = c(0.25), na.rm = TRUE),
            meanMAE = mean(MAE),
            coverage_posterior = mean(coverage_posterior),
            coverage_predictive = mean(coverage_predictive)) %>%
   ggplot(aes(x = lead, color = factor(model, levels = c("Naïve", "Piantham", "MLR","Hierarchical-MLR", "FGA", "GARW"))))  +
  #geom_line(aes(y = quant50), size = 0.5) +
  geom_line(aes(y =coverage_posterior ), alpha = 0.75)+
  ...
  labs(linetype = " ", color = "Model", shape = 1) +
  scale_color_manual(values = model_colors,
                     breaks = c("Naïve", "Piantham", "MLR","Hierarchical-MLR", "FGA", "GARW")) +
  labs(x = "Days from date of estimation", y = "Posterior Coverage (Percent in 95% CI)") +
  scale_x_continuous(breaks = c(-30, -14, 0, 14, 30)) +
  theme(axis.title.x = element_text(color = "Black", size = 14),
        axis.title.y = element_text(color = "Black", size = 14),
        panel.grid.major = element_line(),
  ...
  scale_y_continuous(trans = log_trans(),
                     ...
                     labels = label_percent()) +
    ylim(0,1)+
  annotate(geom = "rect", xmin = -30, xmax = 0, ymin = 0, ymax = 1,
           color = NA, fill = "#bdc3c7", alpha = 0.15) +
  annotate(geom = "text", x = -18, y = 2, label = "Nowcast", size = 3, fontface = 4) +
  annotate(geom = "rect", xmin = 0, xmax = 30, ymin = 0, ymax = 1,
           color = NA, fill = "#2c3e50", alpha = 0.15) +
  annotate(geom = "text", x = 18, y = 2, label = "Forecast", size = 3, fontface = 4)
  #geom_hline(yintercept = 0.05, linetype = "dashed")


# B: coverage predictive

coverage_supp_2_B = error_dataset %>%
  group_by(lead, location, model) %>%
  filter(location %in% c("Australia", "Brazil","Japan","South Africa","Trinidad and Tobago", "United Kingdom","USA", "Vietnam" )) %>%
  filter(model %in% c("Naïve", "Piantham", "MLR","Hierarchical-MLR", "FGA", "GARW"))  %>%
  filter(lead > -30 & lead < 30) %>%
  summarise(quant75 = quantile(MAE, probs = c(0.75), na.rm = TRUE),
   ...
   ggplot(aes(x = lead, color = factor(model, levels = c("Naïve", "Piantham", "MLR","Hierarchical-MLR", "FGA", "GARW"))))  +
  #geom_line(aes(y = quant50), size = 0.5) +
  geom_line(aes(y =coverage_predictive ), alpha = 0.75)+
  ...
  labs(linetype = " ", color = "Model", shape = 1) +
  scale_color_manual(values = model_colors,
                     breaks = c("Naïve", "Piantham", "MLR", "Hierarchical-MLR", "FGA", "GARW", "Heir")) +
  labs(x = "Days from date of estimation", y = "Predictive Coverage (Percent in 95% CI)") +
  scale_x_continuous(breaks = c(-30, -14, 0, 14, 30)) +
  theme(axis.title.x = element_text(color = "Black", size = 14),
        axis.title.y = element_text(color = "Black", size = 14),
        panel.grid.major = element_line(),
...
coverage_supp_2 = ggarrange(coverage_supp_2_A, coverage_supp_2_B, 
          ncol = 1,
                                labels = c("A", "B"), 
                                common.legend = TRUE, 
                                legend = "right")
...
ggsave("coverage_supp_2.png", plot = coverage_supp_2, width = 25, height = 30, units = "cm", bg = 'white',  dpi = 300)
