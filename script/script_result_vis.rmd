...
get_recent_sequences_frac <- function (df, end_date, k) {
  filtered <- df %>%
    ...
    summarise(
      ...
      seq_available = sum(sequences * (as.Date(date_submitted) - as.Date(date) < k)),
...
for (pivot_date in pivot_dates) {
  ...
  seq_d = get_recent_sequences_frac(seq_date, last_date , 14) %>%
...
for (pivot_date in pivot_dates) {
  last_date = as.Date(pivot_date) - 14
  seq_d = get_delays_piv(seq_date, last_date , 14) %>%
    mutate(pivot_date = pivot_date)
...
#Incorporated selective pressure
...
full_test %>%
  filter(model == "MLR") %>%
  ggplot()+
  geom_point(aes(x = seq_collected, y = seq_available, color = location)) +
  facet_wrap(~location, scales = "free") + 
  theme_classic()+
  theme(axis.title.x = element_text(color="Black", size=14),
        axis.title.y = element_text(color="Black", size=14))
...
#Regression Model
reg_US = lm(log(meanMAE) ~ frac_seq_avail + seq_available + lead + median_delay + selective_pres , US_model)
...
#Regression Model
reg_JP = lm(log(meanMAE)~ frac_seq_avail + seq_available + lead + median_delay + selective_pres, JP_model)
...
US_m = ggplot(US_model, aes(x=predict(step.model_US), y= log(meanMAE))) +
...
JP_m =  ggplot(JP_model, aes(x=predict(step.model_JP), y= log(meanMAE))) +
...
#Regression Model
reg_all = lm(log(meanMAE) ~ frac_seq_avail + seq_collected + lead + median_delay + selective_pres, all_count_model)

regress("mean", log(meanMAE) ~ frac_seq_avail + seq_available + lead + median_delay + selective_pres, all_count_model )
...
ggplot(all_count_model, aes(x=exp(predict(reg_all)), y= meanMAE, color= location)) +
  geom_point() +
  facet_wrap(~location)+
  geom_abline(intercept=0, slope=1) +
  theme_bw() #+
