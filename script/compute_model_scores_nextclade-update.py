...
def calculate_errors(merged, pivot_date):
    ...
    error_df["ci_low"] = ci_low if ci_low is not None else np.nan
    error_df["ci_high"] = ci_high if ci_high is not None else np.nan
    return error_df
...
if __name__ == "__main__":
    with open("../config-nextclade-update.yaml", "r") as config:
    ...
    truth_set = load_truthset(
        "../data/real_time_nextclade/truth/seq_counts_truth.tsv"
    )

    # Loading model predictions and computing errors
    score_df_list = []
    for dataset in ["", "_past"]:
        for model in models:
            for location in locations:
                ...
                if len(location_truth) == 0:
                    print(
                        f"{location} is not in the retrospective frequencies."
                    )
                    continue

                for pivot_date in dates:
                    filepath = f"../estimates/real_time_nextclade/nextclade{dataset}/{model}/{location}/freq_full_{pivot_date}.tsv"
                    # if no .tsv, check for .csv
                    if not os.path.exists(filepath):
                        filepath = f"../estimates/nextclade{dataset}/{model}/{location}/freq_full_{pivot_date}.csv"
                    ...
                    # Add column for nextclade version
                    error_df["nextclade_version"] = (
                        "retrospective" if dataset == "" else "real_time"
                    )
    ...
    error_filepath = "../errors/real_time_nextclade"
    if not os.path.exists(error_filepath):
        os.makedirs(error_filepath)
    score_df.to_csv(f"{error_filepath}/model_scores.tsv", sep="\t", index=False)
