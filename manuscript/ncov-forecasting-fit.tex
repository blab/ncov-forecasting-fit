...
\begin{figure}[tb!]
    ...
    (A) Absolute error at nowcast for the MLR model across countries.
		...
		Points represent separate data sets at different analysis dates.
    Correlation strength and significance are calculated via Spearman rank correlation and are inset in each panel.\
		\tbc{Check if Pearson correlation or Spearman rank. I'm assuming you meant Spearman rank given statement of `Pearson rank' and the use of $\rho$ in the figure.}
	}
...
Using Spearman rank correlations \tbc{Again, check this} we find that poor sequence quality as measured by proportion of available sequences labeled as `bad' by Nextclade quality control \cite{aksamentov2021nextclade} correlates slightly with mean AE (Fig.~\ref{fig:vars_of_interest}B).
...
\tbc{In commit f0207d393a30125b24646f92cb7e441516ff39e5 the $R$ correlation coefficient for proportion bad QC when from 0.22 to 0.01. What's going on here? I don't know if I can trust these numbers. It matters for how this paragraph is written.}
...
Additionally, we include Denmark due to its large counts of available sequences and to explore the possibility of stochastic effects due to relative population sizes (Denmark has 30\% the population of the UK).
