...
In observing poor performance in initial period of variant emergence (Fig.\ \ref{fig:ga_estimates}), as well as poor performance in countries with less intensive genomic surveillance (Fig.\ \ref{fig:vars_of_interest}), we conclude lack of data results in poor fitness estimates and so poor predictive performance.
...
We find that the Piantham, MLR, FGA and GARW models provide systematic and substantial improvements in forecasting accuracy relative to a `naive' model that uses 7-day smoothed frequency at the last timepoint with sequence data (Fig.~\ref{fig:model_comp_A}, Table~\ref{table:model_comp_table}).
