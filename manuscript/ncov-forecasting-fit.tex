...
%MF: We did not implement the model for this paper, we used it among several others in this analysis. If we're talking about what's going on in Figure 1, I would say that directly and cite it \ref{fig:Figure1}
...
\begin{figure}[H]
    ...
    %MF: You should also be using things like (A) and (B) to direct the reader
    ...
    %MF: Something similar can be done for (B) Various frequency nowcasts ... are shown ...
    ...
    %MF: Personally, I think it makes most sense to use MLR here for (B) instead of GARW since it is the simplest model.
	}
	\label{Figure1Final}
...
%MF: We did not implement these models for the project. We should say we tested or used 5 models.
These models were implemented using the evofr (evolutionary forecasting) software package in Python. %MF: This can be moved to methods and will need a link to the github
...
%MF: I would flip this slightly e.g. we compare the above 4 models to a naive model to serve as a reference for comparison.
It is a baseline model which uses simple assumptions to make predictions. %MF: Unnescarry
The use of multiple models that ranges in complexity allows for a comprehensive evaluation of the performance and robustness of different forecasting methods. %MF: How so?


%MF: What did we set out to do should come before how we did it.
...
%MF: I would just say: "We used several metrics ...". Otherwise, this bit is near perfect
We used our predictions and implemented a metric from the framework, specifically Mean Absolute Error (MAE), to assess the relative performance of the models for the six countries (as presented in Table 1). %MF: Add a reference to table directly \ref{Table1} and make sure to give it a better name
...
%MF: Do we actually care about the absolute range? The entire table is about the mean MAE, no?
...
%MF: It wasn't the range of values that we used to make this determination, so this sentence is a bit confusing
...
%MF: Something like "The distributions are visualized in \ref{Figure2} A". Once again, make sure to give the figure labels clearer names.
...
\begin{figure}[H]
	...
	\caption{\textbf{MAE estimates based on date of estimation.} %MF: We're predicting. We could also use "Observation date" to remove ambiguity.
	Further legend here.
	}
	\label{Figure 2}
\end{figure}
...
%MF: This is not what is happening in Figure 3 at all... First there's multiple models. Secondly, these are growth advantage estimates as of that day not necessarily the growth advantages on that date.
...
%MF: The growth advantage estimates provided are .... This is also something could appera in the methods or in the figure caption.
...
\begin{figure}[h]
	...
	Further legend here.
    %MF: The document will not render because table1.png wasn't pushed
	}
	\label{table1}
\end{figure}
