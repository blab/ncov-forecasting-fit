...
ncov-forecasting-fit (title TBD) \jhc{What is your primary result? That is your title.}
}
...
% , including Multinomial Logistic Regression (MLR), Fixed Growth Advantage (FGA), Growth Advantage Random Walk (GARW), and Piantham
...
%TODO: Here I would include either the saturation of sequencing on reducing error or the fact that a minimum number of sequences is needed for good reconstruction
% \textbf{Results}:
...
More recently, with the deluge of data from various sources \jhc{which sources?}, issues with data arise regarding quantity \jhc{issue is scaling analyses/storage with increasing data. what else?}, near-real-time accessibility \jhc{availability}, and quality of sequences.
%Integration of these data sources
Data collected from different geographical locations and time-points exhibit varying degrees of missing data, submission delays, and back-filling of disease occurrence especially on larger geographic scales \jhc{What does ``larger geographic scales'' mean here?} \cite{crepey_challenges_2022}.
%preforming real-time analysis
Thus, the quantities and the availability of genomic sequencing data differ depending on the date of observation and geographic region \jhc{This sentence seems redundant with the previous one. Both make great points, but maybe you can condense them into one?}.
...
Thereby, it is essential to take these factors (data-wise and systematic \jhc{What does this mean?}) into account when using mathematical models to make predictions, as they can impact the accuracy and reliability of the model's output \cite{crepey_challenges_2022}.


Mathematical models elucidate disease processes and have been sought to assess the risk \jhc{of} and frame the response to emerging pathogens.
%not mechanisms%
Representation of disease spread from genomic sequences can not \jhc{rephrase as a positive statement?} only forecast disease occurrence and pathogen frequencies \jhc{What are ``pathogen frequencies'' and why do they matter?}, but also uncover the inherent characteristics of emerging pathogens and compare potential mechanisms of spread and persistence in the population \cite{metcalf_opportunities_2017}.
%mention importance of forecasting variant frequences and estimating growth advantage
In particular, forecasting variant frequencies allows us to understand \jhc{perhaps ``interrogate'' instead of ``understand''} the impact of mutation accumulation and viral diversity on the spread of and competition between different variants in the population.
Furthermore, estimating variants' growth advantages allows us to draw inferences regarding a biological selective advantage of specific variants which could potentially result in increased transmissibility or immune evasion \jhc{This statement felt backwards: the increased transmissbility or immune evasion confers the selective advantage not the other way around.} \cite{tegally_detection_2021}.
...
With the emergence of novel pathogens, real-time data scarcity represents a real challenge to accurate forecasts and also nowcasts \jhc{Rewrite the next bit as part of a complete sentence explaining what nowcasts are and how they differ from forecasts.} i.e. forecasting on a short-term scale, as it leads to
...
To investigate how modeling approaches \jhc{Which modeling approaches? Need more background on the models before you describe how you evaluate them.} handle data issues, we developed a framework for evaluating model nowcast and forecast of real-time SARS-CoV-2 \jhc{Why SC2?} variant frequencies within several \jhc{Which?} countries.
...
To assess the performance of these models, we used metrics such as \jhc{I didn't see any other metrics besides MAE emphasized in the current draft} mean absolute error (MAE) to compare the predicted frequencies to a set of known true values (smoothed raw frequency values of SARS-CoV-2 variants \jhc{This parenthetical is a little too detailed for the introduction but is great for the methods}). %what is truth set (Smoothed raw_freq)
This \jhc{what?} allowed us to evaluate the accuracy and reliability of the models and to identify those that were most effective in predicting SARS-CoV-2 circulating variants trends.

%What explains our errors
%try to explain the persistent patterns in the errors
\jhc{The following paragraph read as redundant with the prior paragraph. Could you merge the two into a single paragraph?}
Here, we investigated the persistent patterns of errors in the different models used for nowcasting and forecasting the emergence of SARS-CoV-2 variants.
...
\jhc{If you delete/merge the prior paragraph, then you could merge this sentence with the last remaining paragraph as a closing thought.}
...
Nowcasting variant frequencies is complicated by the continuous \jhc{I think you mean ``ongoing'' here? Continuous sounds like data are continously coming in without any gaps in availability when the reality is a much more discrete and batched submission process for most countries} nature of the data submission process.
...
Utilizing sequencing data for these variants \jhc{Which variants and why?}, we conducted a comparative analysis of the availability of SARS-CoV-2 variants sequences in Japan and the United States \jhc{Why focus on these two countries?} as observed for different observation dates along the period of February 15th to December 1st, 2022 \jhc{Why focus on this date range?} (Figure~\ref{fig:dynamic_forecast_env}A).

\jhc{Merge this paragraph with the preceding?}
Throughout the observation period, the United States had a higher availability of SARS-CoV-2 sequences compared to Japan.
Nonetheless, both countries exhibited variability in sequence availability count for the same dates depending on observation dates \jhc{I didn't follow the last part of this sentence. I generally couldn't tell the difference between observation dates and estimation dates. Are they synonymous?}.
...
We fit the GARW model \jhc{Need to introduce this model by its full name first and also explain what it is. This explanation could go in the introduction.} within the framework \jhc{I didn't understand what ``the framework'' meant. Maybe you can drop the reference?} at each of these observation dates to demonstrate the nature of the real-time forecasting environment for five different SARS-CoV-2 variants (Omicron 21K, 21L, 21A, 22B, 22C) (Figure \ref{fig:dynamic_forecast_env}.B).
...
However, we observed that the model predictions for Japan were more frequently misestimated compared to United States \jhc{Which data support this statement? The nowcasts shown in Figure 1B look generally similar between the two countries. Maybe you could expand on a couple of specific examples from the figure here to support this statement. For example, compare frequency estimates for clades 22B and 22E.}.
...
\begin{figure}[H]
	...
	\caption{\textbf{Example \jhc{``Representative'' might be a better word here. ``Example'' suggests toy or simulated data.} data and predictions for Japan and USA.}
	...
	(B) Various \jhc{Which?} frequency nowcasts, which depends on the data, vary by observation date.
    \jhc{Consider adding a legend title of ``Nextstrain clade'' and then you can drop the ``Clade'' prefix from all of the clade labels. Also, I couldn't tell what the different lines in each panel of part B represented. Can you describe what those are in the caption?}
    }
...
We began our \jhc{model} comparison by calculating the lag times \jhc{define these first and then name them instead of placing definition in a parenthetical} (difference in days between the date of observation and the date of estimation) as a means of evaluating the performance of the models over time.
\jhc{Model comparison is per model, per variant, per country, per observation/estimation date, right?}
This approach provides a reference for understanding how the models perform as the lag time \jhc{Here you refer to ``lag time'', but later in the paper it is ``lead time''.} decreases from the forecast period to the nowcast period, where the lag time approaches zero (the date of estimation), specifically from [-30,30] \jhc{What does this range mean? Specifically, what does a lag/lead time of ``-30'' mean in practical terms?}.
This \jhc{What?} enables us to evaluate the effectiveness of the models as we move closer to and further away from the date of observation.
...
The first four models are GARW, FGA, MLR, and Piantham \jhc{define these earlier without acronyms and include citations}, which are evolutionary forecasting models varying in complexity, refer to methods for more details.
...
We used our predictions and used several \jhc{Which?} metrics from the framework, specifically mean absolute error (MAE), to assess the relative performance of the models for the six countries (Table~\ref{table:model_comp_table}).
...
There was a range of values \jhc{What was the range?} among the models for different leads.
...
In other words, as forecasting step increases, the prediction accuracy for all models decreased \jhc{Maybe ok to drop this sentence as a bit redundant?}.
...
\paragraph{Comparison of Growth Advantages}\ We investigated the behavior of the growth advantage of different variants in various \jhc{Which?} countries.
Using GARW, MLR, and Piantham models \jhc{Why these?}, we standardized the varying growth advantage by estimating the median growth advantage values as of that date, \jhc{The following should be a separate sentence.} we were able to observe when and which variants stabilized in each country (Figure~\ref{fig:ga_estimates}).
The majority of countries displayed stabilization with regard to the Omicron 22A, B, and C variants \jhc{Mixing Nextstrain clades with WHO nomenclature here}, with the exception of Japan.
This discrepancy may be attributed to Japan's limited availability of sequence data or other imperfections in the data \jhc{What kinds of imperfections?}.
...
\paragraph{Effects of changing sequencing intensity of \jhc{on?} nowcast error}%
...
We prepare sequence count datasets to replicate a live nowcasting environment.
For a given pivot \jhc{define ``pivot'' or drop in favor of ``observation date'' alone} or observation date, we filter to all sequences \jhc{from which database(s)} which were collected 90 days before the observation date.
To properly account for submission delays in this collection process, we additionally filter to those sequences which were submitted before the observation day.
These sequences are then counted according to their Nextstrain clade assignment \jhc{cite Nextstrain clade definitions here. How did you assign clades for this project?} to produce sequence counts for each Nextstrain clade each day over the period of interest.
...
We repeat this process for a series of observations days which are the 1st and 15th of each month starting with  Janurary 1st, 2022 and ending with December 15th, 2022 giving a total of 24 datasets for each country.
...
We can interpret these transmission advantages as the relative effective reproduction number of a variant against a given pivot \jhc{Pivot used here with a different meaning than the previous methods section.} variant.
...
The models differ in assumptions in computing this quantity \jhc{Write out as a full sentence} i.e. MLR depends on an assumption of a fixed generation time, Piantham assumes the total prevalence is fixed in time, while GARW / FGA additionally model the transmission process using case count data.
Further details on the model formats can be found in their respective citations.
All models were implemented using the evofr (evolutionary forecasting) software package in Python \jhc{Add link to PyPI package?} using Numpyro for inference.
...
\jhc{I never saw RMSE or LL used in the paper. Drop these here and elsewhere?}

\paragraph{Generating predictors of error}

We explore four key variables to better encompass \jhc{?} the effect of sequencing efforts which are fraction of sequences available, sequences available, median delay and lead time.
...
\jhc{Throughout the paper you use ``downscale'' and ``subsample''. Maybe ``downsample'' is more appropriate instead?}
...
Using sequences from the United Kingdom, we subsample existing sequences at a rate of 100, 500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000 sequences per week at various \jhc{which?} observation dates throughout the pandemic.
